...
- **KAR-414** **Reads public, writes gated on the deployed docket.** The service is deployed `--allow-unauthenticated` deliberately — the challenge box is the most direct demonstration this project has and must work with no login and no quota. But `/edit` and `/ratify` append events and trigger a Drive write, and unauthenticated *reads* must not silently mean unauthenticated *writes*: anyone with the URL could otherwise append an event carrying `actor: "instructor"`. An instructor token, exchanged once for an `HttpOnly` cookie, gates exactly those two endpoints; with no token configured they stay open so the local demo and the recorded video are unchanged. *AC (property: anonymous writes append nothing):* with a token configured, an anonymous POST to `/edit` leaves the event count unchanged and `/ratify` returns 403, while `/`, `/challenge`, `/student/{id}` and `/appeal/{id}` all return 200.

- **KAR-415** **The instructor records a grade, through the docket, into the grades database.** `GradesStore.write()` existed with no caller, which meant the centrepiece of the security argument had never been written to by the running system: the boundary was demonstrated only in the negative — a service account being denied — which shows an identity cannot write, not that the destination is real. A token-gated `POST /grade` takes the value as typed input from a person; there is no constructor path from an `Observation` to a `Grade`, and none may be added. *AC (property: no derivation path exists):* `karani.grades` imports nothing observation-shaped and no function in it accepts an observation, asserted by parsing the module rather than by grepping its prose; and an unreachable grades database produces an explicit failure, never a silent success.

- **KAR-416** **The glass-box replay.** `/replay` steps through the committed event log in fold order — tiles accumulating, ticker scrolling — because "it ran unattended" is an assertion and forty seconds of watching consequences differ is the receipt. Only event metadata (step, item id, attempt, timestamp) ships to the browser; payloads carry generated text and never leave the server. *AC (property: no generated prose can reach the page):* every replay row has exactly the four metadata fields, and no observation text from the run occurs anywhere in the served bytes.
- **KAR-417** **The cross-family second reader.** Layer 5: `gemma3:4b` via local Ollama re-answers the entailment question for every citation the Gemini verify tier accepted. Entailment is the one layer where the checker could share the generator's blind spots; a second model family narrows that class. Disagreement escalates and is never retried (KAR-310's reason); unavailability records `verification.second_reader: null` — not run, never a pass; every response caches through the standard `ResponseCache` so offline replay serves real recorded Gemma output. Off by default so previously published numbers are untouched. *AC (property: the check cannot fail toward approval):* an unreachable Ollama with an empty cache yields `checked=False, confirmed=False`, asserted by a test that survives only if the honest branch exists.
- **KAR-418** **The morning brief.** After the unattended run, Karani delivers the work-list unasked: what needs the instructor (escalations with reasons), what is done (outcome counts), and class-level reteach patterns — "N of M submissions drew no evidence on this criterion," with cited examples in the students' own words. Ships with every ratified Drive delivery and serves at `/brief`. A pure function of the fold; no model writes the brief. *AC (property: aggregates never become judgments):* the reteach threshold is a count (a third of the class, minimum two), examples are quotes or linted notes, and no characterization vocabulary appears on either corpus's brief.

- **KAR-419** **The arena: bring your own essay.** A public, unauthenticated page where a visitor pastes any text and the **genuine pipeline** runs on it — rendition freeze, span registry, injection scan, live Gemini analysis, all validation layers — returning the evidence sheet an instructor would see, with no grade because there is still nowhere to put one. Maximal honesty through maximal reuse: the arena contains no analysis code of its own, calling the same `run_pipeline` as the nightly job (asserted structurally by test). Runs are **ephemeral** — a stranger's text is not this system's to keep: the corpus, events, and cache live and die inside one TemporaryDirectory. Limits are honest about their own weakness (in-memory, per-instance, reset on cold start; the real backstops are the character cap and budget alerts). *AC (property: the demo is the system):* `_analyse` calls `run_pipeline` and defines no prompt or model call of its own; the temp directory is gone after the request; a fourth request in an hour is refused before any model call. The arena's first live test found a real defect — an injection wrapped across a soft line break evaded the scanner — which is the strongest argument for the page existing.

- **KAR-420** **The boundary, as a page.** `/boundary` lets anyone press a button and watch this service attempt a fresh grade-record create under its own identity, live, and be refused by IAM before Karani's code gets a say. Exists because the denial was previously provable only in a terminal — true, and unreadable to the instructor it protects. Locally (no cloud credentials) the page says the attempt could not run from here rather than staging a fake refusal; if the write ever succeeds, the page reports BOUNDARY FAILURE at maximum volume instead of styling it away. The deployed pytest gate remains the release proof. *AC:* credential-less attempt renders the honest could-not-attempt message and nothing that reads as success.
- **KAR-421** **The gradebook shown, not exported into the void.** The delivery receipt renders the CSV inline with the empty grade cells visible ("— yours to write —"), because the one fact the system is arranged around deserves to be looked at, and the hosted container's filesystem is browsable by nobody. *AC:* the receipt table's grade column renders empty for every ungraded student.
- **KAR-422** **The second domain, one click away.** `/scholarship` serves the committed scholarship run — real Gemini analysis and real local-Gemma second-reader verdicts, replayed from the fixture log baked into the image — as a read-only exhibit with navigation rebased into itself and all write actions disabled. *AC:* exhibit serves without a token; internal links stay inside `/scholarship/`; write posts return 403 even with the instructor token.
